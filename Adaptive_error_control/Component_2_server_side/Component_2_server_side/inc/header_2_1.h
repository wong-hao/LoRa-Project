...
public:
  ...
  char *inter; //对于Buffer来说是将bufferi_inter赋值buffer_inter给以后续处理 /
               //对于BufferSend来说是Upstream JSON data structure
  char *inter_backup; //对于Buffer来说是复制一份inter防止deleteChar重复执行多次
  uint8_t *inter_uint;
  char *inter_uint_char; //需要发送的数据的char形式（此时前12-byte
                         //header有缺陷，第12 byte后为修改后的Upstream JSON data
                         //structure）
  char send_first_part_char[BUF_SIZE]; //对于BufferSend来说是12-byte header
  char send_last_part_char[BUF_SIZE]; //对于BufferSend来说是修改后的Upstream
                                      //JSON data structure
  uint8_t *send; //对于BufferSend来说是需要发送的数据 (原始uint8形式)

  uint8_t payload[BUF_SIZE];
      /*!> buffer containing the payload */ // json里的"data"使用b64_to_bin解码得到的LoRaWAN
                                            // Frame里的PHY payload
  uint16_t size;   // json数据包里自带的，但mqtt event没有
  char *Hexstring; //对于Buffer来说PHY payload的char形式 /
                   //对于BufferSend来说是char类型的realresult组成的新PHYPayload
  char *Binarystring; //对于BufferSend来说是PHY payload的二进制char形式
  char *Binarystring2; //对于BufferSend来说是Merged error mask / Ambiguity
                       //vectors / Va
  ...
  void setData(char *array) { strcpy(data, array); }

  void setIndex() { index = strlen(data) / 2; }

  void setUint() { Char2Uint(data, uint); }

  virtual void setInter() { inter = (char *)(uint + buff_index); }

  void setInter_Uint() { inter_uint = (uint8_t *)(inter - buff_index); }

  void setSize(const char *str) {
    size = b64_to_bin(str, strlen(str), payload, sizeof payload);
  }

  virtual void setHexstring() { Uint2Char(payload, Hexstring, size); }

  virtual void setBinarystring() { Hex2Bin(Hexstring, Binarystring); }
};
...
public:
  ...
  void setInter(char *array) { strcpy(inter, array); }

  void setHexstring(char *array) { Bin2Hex(array, Hexstring); }
  ...
  void setForthBinarystring(char *array1, char *array2, char *array3,
                            char *array4) {
  ...
  void setSend_Last_Part_Char() {
    strncpy(send_last_part_char, inter_uint_char + buff_index * 2,
            strlen(inter_uint_char) - buff_index * 2);
  ...
  void setHexstring_uint() { Char2Uint(Hexstring, Hexstring_uint8); }
};
...
public:
  ...
  void setDevAddr_get(uint8_t array[BUF_SIZE], int num) {
    DevAddr_get = (int)json_value_get_number(json_object_get_value(
        json_array_get_object(
            json_object_get_array(
                json_value_get_object(json_parse_string_with_comments(
  ...
  void setFcnt(uint8_t array[BUF_SIZE], int num) {
    fcnt = (int)json_value_get_number(json_object_get_value(
        json_array_get_object(
            json_object_get_array(
                json_value_get_object(json_parse_string_with_comments(
                    (const char *)(array + num))),
  ...
  void setStat(uint8_t array[BUF_SIZE], int num) {
    stat = (int)json_value_get_number(json_object_get_value(
        json_array_get_object(
            json_object_get_array(
                json_value_get_object(json_parse_string_with_comments(
                    (const char *)(array + num))),
  ...
  void setCrc_get(uint8_t array[BUF_SIZE], int num) {
    crc_get = (int)json_value_get_number(json_object_get_value(
        json_array_get_object(
            json_object_get_array(
                json_value_get_object(json_parse_string_with_comments(
                    (const char *)(array + num))),
  ...
  void setRssi(uint8_t array[BUF_SIZE], int num) {
    rssi = (int)json_value_get_number(json_object_get_value(
        json_array_get_object(
            json_object_get_array(
                json_value_get_object(json_parse_string_with_comments(
                    (const char *)(array + num))),
  ...
  void setSNR(uint8_t array[BUF_SIZE], int num) {
    snr = (float)json_value_get_number(json_object_get_value(
        json_array_get_object(
            json_object_get_array(
                json_value_get_object(json_parse_string_with_comments(
                    (const char *)(array + num))),
  ...
  void setPayloadSize(uint8_t array[BUF_SIZE], int num) {
    PayloadSize = (int)json_value_get_number(json_object_get_value(
        json_array_get_object(
            json_object_get_array(
                json_value_get_object(json_parse_string_with_comments(
                    (const char *)(array + num))),
...
int compareTime(Rxpk *rxpk_array,
                int array_length); //防止fcnt与CRC一样出现错误导致无法一起处理

int compareFcnt(
    Rxpk *rxpk_array,
    int array_length); //防止网关接收时间相差一秒导致无法一起进行处理