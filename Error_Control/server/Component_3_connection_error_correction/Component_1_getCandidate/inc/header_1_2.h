...
#define MAXLATENCY 0.218           //computation overhead  (confirmed: RECEIVE_DELAY1 > Lantency + TOA / RECEIVE_DELAY2作为备用)， 这样才能体现出三个stage组合的作用: 降低空间时间复杂度
...
void incremental_correct(char *input, char *mch, int Hamming_weight_now, int crc_int, char *fakeresult, char (*realresult)[BUF_SIZE], int length, int &pass_crc, int &total_number, struct timespec startTime, int fcnt, u4_t devaddr);//deprecated because of invoked oom-killer
