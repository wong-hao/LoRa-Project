...
def drawEffThroughput():
    ...
    for loopcount in range(n * SFNum * TPLevelNum):
        ...
        if loopcount % (n * TPLevelNum) == 0:
            TP0data[0].append(loadCSFinalThroughput(CSParaDataset[loopcount]))
        elif loopcount % (n * TPLevelNum) == 1:
            TP0data[1].append(loadCSFinalThroughput(CSParaDataset[loopcount]))
        elif loopcount % (n * TPLevelNum) == 2:
            TP0data[2].append(loadCSFinalThroughput(CSParaDataset[loopcount]))
        elif loopcount % (n * TPLevelNum) == n:
            TP1data[0].append(loadCSFinalThroughput(CSParaDataset[loopcount]))
        elif loopcount % (n * TPLevelNum) == n+1:
            TP1data[1].append(loadCSFinalThroughput(CSParaDataset[loopcount]))
        elif loopcount % (n * TPLevelNum) == n+2:
            TP1data[2].append(loadCSFinalThroughput(CSParaDataset[loopcount]))
        elif loopcount % (n * TPLevelNum) == 2*n:
            TP2data[0].append(loadCSFinalThroughput(CSParaDataset[loopcount]))
        elif loopcount % (n * TPLevelNum) == 2*n+1:
            TP2data[1].append(loadCSFinalThroughput(CSParaDataset[loopcount]))
        elif loopcount % (n * TPLevelNum) == 2*n+2:
            TP2data[2].append(loadCSFinalThroughput(CSParaDataset[loopcount]))
