...
from src.tool.formatnum import Millisecond2Second
...
def drawInstantThroughput():
    ...
    # Initialize axis
    ax1.set_ylim((0, 0.0224))
    # ax1.ticklabel_format(axis="both", style="sci", scilimits=(0, 0))
    ax1.set_xlabel(r'Time (s)', fontsize=15)
    ax1.set_ylabel(r'Throughput (kbps)', fontsize=15)

    # Draw lines
    ax1.plot(Millisecond2Second(x1), y1, color='r', label=r'Central Server')
    ax1.plot(Millisecond2Second(x2), y2, color='b', label=r'Application Server', linestyle='dashed')
